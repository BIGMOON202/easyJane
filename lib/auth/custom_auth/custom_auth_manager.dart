...
class CustomAuthManager {
  ...
  Future signOut() async {
    ...
    easyJaneAuthUserSubject.add(
      EasyJaneAuthUser(loggedIn: false),
    );
    persistAuthData();
  }

  Future<EasyJaneAuthUser?> signIn({
    String? authenticationToken,
    String? refreshToken,
    DateTime? tokenExpiration,
    String? authUid,
  ...
  EasyJaneAuthUser? _updateCurrentUser({
    String? authenticationToken,
    String? refreshToken,
    DateTime? tokenExpiration,
    String? authUid,
    ...
    final updatedUser = EasyJaneAuthUser(
      loggedIn: true,
      uid: authUid,
      userData: userData,
    );
    easyJaneAuthUserSubject.add(updatedUser);
    persistAuthData();
    return updatedUser;
  }
  ...
  Future initialize() async {
    ...
    try {
      ...
      userData = _prefs.getString(_kUserDataKey) != null
          ? LoginResposeStruct.fromSerializableMap(
